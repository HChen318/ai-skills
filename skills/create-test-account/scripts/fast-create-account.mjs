 ...
 * Usage:
 *   node .agents/skills/create-test-account/scripts/fast-create-account.mjs [--level 0|1|2]
 * 
 * Target Levels:
 *   --level 0 : Pure API account registration + phone binding (~1.5 seconds)
 *   --level 1 : Standard KYC Level 1 (~15 seconds)
 *   --level 2 : Advanced KYC Level 2 (~30 seconds, default)
 */
...
const t1 = Date.now()
const task = await useOrCreateTaskSpace('turbo-kyc-' + randomNum)
