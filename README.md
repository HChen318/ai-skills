...
### ✨ 为什么不需要多余的胶水代码？
- **自包含与单一事实源**：AI Agent（Cursor / CodeX / Antigravity）原生具备阅读 `SKILL.md`、理解上下文并直接调度 `scripts/` 下脚本的能力。
...
| 技能名称 | 目录 | 功能描述 | 耗时 |
| :--- | :--- | :--- | :--- |
| **`create-test-account`** | [`skills/create-test-account`](./skills/create-test-account/SKILL.md) | 3World 测试环境账号极速生成与 KYC 认证（支持普通账号、标准 Level 1、高级 Level 2 认证） | **~1.5s ~ 45s** |
...
### 1️⃣ 在 Cursor / CodeX 中使用

安装后，无论在 **Cursor Composer / Agent 模式**，还是在 **CodeX** 对话中，直接使用自然语言对话即可：
...
若需跳过 AI 对话在本地脚本/CI 中批量创建账号，可直接运行技能脚本：
...
# 生成基础注册账号（Level 0，仅注册+绑手机，耗时约 1.5 秒）
...
| 认证级别 | 操作内容 | 优化前耗时 | 优化后耗时 | 提速效果 | 账户限额 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Level 0 (普通账号)** | 纯 API 注册 + 邮箱验证 + 绑定 `+852` 手机号 | ~180s | **~1.5 秒** | **⚡ 99.2%** | 基础权限 |
| **Level 1 (标准认证)** | Level 0 + 自动上传德国身份证正反面完成认证 | ~240s | **~15 - 20 秒** | **⚡ 92.5%** | 每日 **10,000 USDT** |
| **Level 2 (高级认证)** | Level 1 + 自动上传地址证明账单完成高级认证 | ~353s (5m 53s) | **~35 - 45 秒** | **⚡ 88.7%** | 每日 **50,000 USDT** |
...
1. **Node.js**：`>= 18.0.0`
2. **ego-browser**：用于浏览器自动化（涉及 Sumsub SDK 页面元素操作与文件注入）。
